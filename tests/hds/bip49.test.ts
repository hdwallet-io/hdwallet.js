...
import { BIP49HD, HDS } from '../../src/hds';
import { BIP49Derivation } from '../../src/derivations';
...
describe('BIP49HD', () => {
  const root = rawVectors['BIP49'];
  const drv = rawVectors['BIP49']['derivation'];

  it('initializes from seed and exposes root values', () => {
    const hd = new BIP49HD({
    ...
    expect(hd).toBeInstanceOf(BIP49HD);
    expect(hd.getName()).toBe(root['name']);
    expect(hd.getSeed()).toBe(root['seed']);

    // BIP49 uses its own (ypub/yprv) versions by default
  ...
  it('derives via BIP49Derivation and exposes child values', () => {
    const hd = new BIP49HD({
      ecc: Cryptocurrency.ECC,
      wifPrefix: Cryptocurrency.NETWORKS.MAINNET.WIF_PREFIX
    });
    ...
    const der = new BIP49Derivation({ coinType: Cryptocurrency.COIN_TYPE });
    der.fromAccount(0);
    ...
    // Address (BIP49 → P2WPKH-in-P2SH). TS API: pass scriptAddressPrefix.
    expect(
      hd.getAddress({
  ...
  it('registry returns the same class as a direct import', () => {
    ...
    expect(RegistryClass).toBe(BIP49HD);
  });
  ...
  it('registry-created and direct instances expose the same derived child values', () => {
    ...
    const d1 = new BIP49Derivation({ coinType: Cryptocurrency.COIN_TYPE });
    d1.fromAccount(0);
    d1.fromChange('internal-chain' as any);
    d1.fromAddress(0);

    const d2 = new BIP49Derivation({ coinType: Cryptocurrency.COIN_TYPE });
    d2.fromAccount(0);
