...
import { DERIVATIONS, BIP49Derivation } from '../../src/derivations';
...
describe("BIP49Derivation", () => {
  const def = rawVectors.BIP49.default;
  const from = rawVectors.BIP49.from;

  it("initializes with defaults", () => {
    const d = new BIP49Derivation();
  ...
  it("rejects invalid change at construction", () => {
    expect(() => new BIP49Derivation({ change: "invalid-change" as any })).toThrow(DerivationError);
  });
