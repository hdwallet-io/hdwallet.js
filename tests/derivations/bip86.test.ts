...
import { DERIVATIONS, BIP86Derivation } from '../../src/derivations';
...
describe("BIP86Derivation", () => {
  const def = rawVectors.BIP86.default;
  const from = rawVectors.BIP86.from;

  it("initializes with defaults", () => {
    const d = new BIP86Derivation();
  ...
  it("rejects invalid change at construction", () => {
    expect(() => new BIP86Derivation({ change: "invalid-change" as any })).toThrow(DerivationError);
  });
