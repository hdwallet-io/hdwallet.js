...
describe("BIP49.fromXPrivateKey", () => {
  it("compressed", () => {
    const compressed = data.BIP49['compressed'];
    const cryptocurrency = CRYPTOCURRENCIES.getCryptocurrencyClass(compressed['cryptocurrency']);
    const DerivationClass = DERIVATIONS.getDerivationClass(data.BIP49.derivation.name);

    const derivation = new DerivationClass({
      coinType: data.BIP49.derivation.args['coin-type'],
      account: data.BIP49.derivation.args['account'],
      change: data.BIP49.derivation.args['change'] as any,
      address: data.BIP49.derivation.args['address']
    });
    ...
    const hdwallet = new HDWallet(cryptocurrency, {
      ...
      .fromXPrivateKey(compressed['root-xprivate-key'], compressed['strict'])
  ...
  it("uncompressed", () => {
    const uncompressed = data.BIP49['uncompressed'];
    ...
    const hdwallet = new HDWallet(cryptocurrency, {
      ...
      .fromXPrivateKey(uncompressed['root-xprivate-key'], uncompressed['strict'])
      .fromDerivation(derivation);
