...
describe("Cardano.ByronLedger.fromXPrivateKey", () => {
  it("works", () => {
    const src = data.Cardano['byron-ledger'];
    ...
    const hdwallet = new HDWallet(cryptocurrency, {
      ...
      addressType: (cryptocurrency as any).ADDRESS_TYPES.PUBLIC_KEY
    })
      .fromXPrivateKey(src['root-xprivate-key'], src['strict'])
