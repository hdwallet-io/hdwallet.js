...
describe("Cardano.ShelleyIcarus.fromEntropy", () => {
  it("works", () => {
    ...
    const hdwallet = new HDWallet(cryptocurrency, {
      ...
      .fromEntropy(new BIP39Entropy(src['entropy']))
    ...
    expect(hdwallet.getAddress({ addressType: (cryptocurrency as any).ADDRESS_TYPES.STAKING })).toBe(last['address']);
